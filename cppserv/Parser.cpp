...
//COMMAND
//:PARAM PARAM
struct Command::Data	Parser::parseClientMessage(const std::string& message) throw(Error)
{
	struct Command::Data						command_data;
	std::vector<struct Parser::Token>			tokens = Parser::splitTokens(message);
	std::vector<struct Parser::Token>::iterator	it = tokens.begin();

	if (tokens.size() < 2)
	{
		throw Error(EWRARG, message.c_str());
	}
...
//	try
//	{
//		struct Command::Data	command = Parser::parseClientMessage(":PREFIX    COMMAND    hello1\r\n");
//
