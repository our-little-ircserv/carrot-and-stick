...
void	IRC::init() throw(Signal, FatalError)
{
	...
	EV_SET(&event, _server_sockfd, EVFILT_READ, EV_ADD, 0, 0, (void*)(&_server_event_handler));
...
void	IRC::deliverMsg(std::set< Client* >& target_list, std::string msg)
{
	...
	for (; it != ite; it++)
	{
		...
		if ((*it)->_writable == false)
		{
			...
			EV_SET(&t_event, (*it)->getSocketFd(), EVFILT_WRITE, EV_ENABLE, 0, 0, (void*)(&_server_event_handler));
...
//
//	EV_SET(&t_event, client.getSocketFd(), EVFILT_READ, EV_ADD, 0, 0, (void*)(&_client_event_handler));
//	server._changelist.push_back(t_event);
//
//	EV_SET(&t_event, client.getSocketFd(), EVFILT_WRITE, EV_ADD | EV_DISABLE, 0, 0, (void*)(&_client_event_handler));
