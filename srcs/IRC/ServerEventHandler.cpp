...
void	ServerEventHandler::read(IRC& server, const struct kevent& event) throw(Signal, FatalError, enum Client::REGISTER_LEVEL)
{
	...
	server.createClient(client_fd, client_addr);

	struct kevent		t_event;
	EV_SET(&t_event, event.ident, EVFILT_READ, EV_ADD, 0, 0, reinterpret_cast< void* >(&(server.getClientEventHandler())));
	server.pushEvent(t_event);

	EV_SET(&t_event, event.ident, EVFILT_WRITE, EV_ADD | EV_DISABLE, 0, 0, reinterpret_cast< void* >(&(server.getClientEventHandler())));
	server.pushEvent(t_event);
}
